...
public sealed class YtDlpService
{
    ...
    public async Task<VideoInfo> GetInfoAsync(string url, CancellationToken token,
        string? cookiesPath = null, string? browserCookies = null)
    {
        var args = new List<string>
        {
            "--encoding", "utf-8", "--dump-single-json", "--no-playlist", "--skip-download", "--no-warnings"
        };
        AddYouTubeAccessArguments(args, cookiesPath, browserCookies);
        args.Add(url);
        ...
        IProgress<string> log, IProgress<DownloadProgress> progress, CancellationToken token, ClipSelection? clip = null, double? sourceDuration = null, string encoder = "cpu")
    {
        ...
        if (clip != null)
        {
            ...
            var sourceInfo = await GetInfoAsync(url, token, cookiesPath, browserCookies);
    ...
    private void AddYouTubeAccessArguments(List<string> args, string? cookiesPath, string? browserCookies)
    {
        ...
        else if (!string.IsNullOrWhiteSpace(browserCookies) && browserCookies != "none")
        {
