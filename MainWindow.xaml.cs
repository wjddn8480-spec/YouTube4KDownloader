...
public partial class MainWindow : Window
{
            ...
            while (true)
            {
                ...
                foreach (var item in pendingItems)
                {
                    ...
                    try
                    {
                        ...
                        item.Title = string.IsNullOrWhiteSpace(info.Title) ? item.Url : info.Title;
                    }
                    catch { info = new VideoInfo { Title = item.Url }; }

                    item.Status = T("DownloadInProgress");
                    QueueGrid.Items.Refresh();
                    var progress = new Progress<(double Percent, string Status)>(v =>
                    {
                        item.Progress = v.Percent;
                        item.Status = v.Status;
                        DownloadProgressBar.Value = v.Percent;
                    ...
                    {
                        _history.Insert(0, new DownloadHistoryItem { DownloadedAt = DateTime.Now, Title = item.Title, Url = item.Url, Format = OutputFormats.DisplayName(SelectedFormat()), Folder = OutputFolderTextBox.Text.Trim(), Success = success });
    ...
    private void SetBusy(bool busy, string status) { _isBusy = busy; FormatComboBox.IsEnabled = !busy; InspectButton.IsEnabled = !busy; UpdateToolsButton.IsEnabled = !busy; StartQueueButton.IsEnabled = !busy; AddQueueButton.IsEnabled = !busy; CancelButton.IsEnabled = busy; StatusText.Text = status; }
