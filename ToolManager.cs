...
public sealed class ToolManager
{
    ...
    private static string L(string ko, string en) => LocalizationManager.CurrentLanguage == "ko" ? ko : en;
    private static HttpClient CreateClient()
    {
        var client = new HttpClient { Timeout = TimeSpan.FromMinutes(20) };
        client.DefaultRequestHeaders.UserAgent.ParseAdd("YouTube4KDownloader/1.3.3");
        return client;
    }
    ...
    public async Task EnsureToolsAsync(IProgress<string>? progress = null, bool forceUpdate = false, CancellationToken token = default)
    {
        ...
        try
        {
            bool yt = forceUpdate || !File.Exists(YtDlpPath);
            ...
            if (yt)
            {
                progress?.Report(L("yt-dlp 업데이트 확인 중...", "Checking yt-dlp updates..."));
            ...
            if (ff)
            {
                progress?.Report(L("FFmpeg / FFprobe 업데이트 확인 중...", "Checking FFmpeg / FFprobe updates..."));
            ...
            catch (Exception ex) { progress?.Report(L("업데이트 캐시 저장 실패: ", "Could not save update cache: ") + ex.Message); }
            progress?.Report(files.Count == 0 ? L("도구가 최신 상태입니다.", "Tools are up to date.") : L("도구 업데이트 완료.", "Tools updated."));
        }
        finally
        {
